...
if plotFlag == 1
    ...
    fig = figure('Position',[0 0 2000 500],'Visible','Off'); hold on;
    i = 0;
    for j = 10:10:90
        i = i+1;
        cmd = ['current_var = prc_',num2str(j),';']; eval(cmd);
        plot(matlab_dates,current_var,'LineWidth',2,'Color',c(i,:,:));
        plot_legend{i} = [num2str(j),'th Percentile'];
    end
    plot(matlab_dates,prc_50,'LineWidth',3,'color','y');
    plot(matlab_dates,prc_50,'LineWidth',2,'color','k','LineStyle',':');
    s = scatter(matlab_dates,prc_50,30,'w','filled');
    ...
    ylim([0.2 1])
    xlim([matlab_dates(3) max(matlab_dates)])
    title('Quebrada del Toro')
